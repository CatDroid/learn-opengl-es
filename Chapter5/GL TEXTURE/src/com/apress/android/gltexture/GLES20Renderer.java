...
//		Matrix.multiplyMM(_MVPMatrix, 0, _ProjectionMatrix, 0, _MVPMatrix, 0);
		...
		Matrix.setIdentityM(_MVPMatrix, 0); // 不做任何的MVP转换
...
//		GLES20.glUniform1i(_planeUSamplerLocation, 0);
        ...
        GLES20.glVertexAttribPointer(_planeACoordinateLocation, 2, GLES20.GL_FLOAT, false, 4*2, _planeTFB);
	 ...
	 */
		...
		float[] planeTFA = {
				...
				// 0.5f,0.5f,     0,1,      3,0,          0,0   
				 0.5f,0.5f,     0,1,      2,0,          0,0   // 留意 顶点坐标的方向 和  纹理方向:如果顶点方向和纹理坐标方向相反 会导致图片相反  (使用planeVFAr 不做MVP)
				// 0.5f,0.5f,     0,2,      2,0,          0,0
		};
		/* 
...
//		_planeVFB.position(0);
		...
		ByteBuffer planeVBB = ByteBuffer.allocateDirect(planeVFAr.length * 4);
		planeVBB.order(ByteOrder.nativeOrder());
		_planeVFB = planeVBB.asFloatBuffer();
		_planeVFB.put(planeVFAr);
		_planeVFB.position(0);	

...
public class GLES20Renderer implements Renderer {
	...
	public static volatile List<Missile> _missiles			= new ArrayList<Missile>(100);
	public static volatile List<Enemy> _enemies				= new ArrayList<Enemy>(10);
	...
	public void onDrawFrame(GL10 gl) {
		...
		if(GLES20Renderer._enemies.size() > 0) {
			...
			while(enemiesIterator.hasNext()) {
				...
				if((enemyX > -20 && enemyX < 0) && (enemyY > -20 && enemyY < 0)) {
					...
					if(GLES20Renderer._missiles.size() > 0) {
						...
						while(missilesIterator.hasNext()) {
							...
							float[] missileCenter	= new float[]{missile.getDestinationPositionX(),missile.getDestinationPositionY(),0};
							// change the coordinate w.r.t global center, instead of {10.0005, 10.0, 0.1005}
							float[] difference		= new float[]{missileCenter[0]-(enemyX+10),missileCenter[1]-(enemyY+10),0};
							if(Matrix.length(difference[0], difference[1], 0) < 3) {
								renderEnemy			= false;
								missilesIterator.remove();
								enemiesIterator.remove();
								// using break to exit while(missilesIterator.hasNext()) loop
								break;
							}
						}
					}
				...
				if(renderEnemy) {
					...
					Matrix.translateM(enemiesMMatrix, 0, enemyOX, enemyOY, 0);
